...
- Safe authored copy helpers: `text(...)` for submitted/plain values and `md(...)` / `markdown(...)` for rendered display prose.
...
TrustedForm display copy can be static strings or `md(...)` values, and dynamic display copy must be returned from the step-level resolver with `md(...)`. Native controls, route slugs, keys, review field labels, and submitted values stay plain text; resolver-produced submitted values use `text(...)`.
