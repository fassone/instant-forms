...
export function registerAssetRoutes(app: Hono): void {
  ...
  app.get("/_instant/forms/:hash/transition.json", async (c) => {
    ...
    const bundleFile = Bun.file(path.join(process.cwd(), DIST_FORMS_ROOT, "_instant", "forms", hash, "transition.json"));

    if (!(await bundleFile.exists())) {
      return new Response("Not found", { status: 404 });
    }

    return assetResponse(bundleFile, "application/json; charset=utf-8");
  });
}
