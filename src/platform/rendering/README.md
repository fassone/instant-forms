...
When a flow enables Google Tag Manager, rendering owns the head bootstrap: first-party request shims are installed from the authored proxy registry, `dataLayer` is created, Partytown is configured to forward `dataLayer.push`, the tiny Partytown bootstrap runtime is inlined, GTM debug bootstrap is excluded from worker execution, and the first-party GTM script URL is rendered.

Tracking events are authored by the form DSL. Rendering and client behavior only emit lifecycle events that the flow declared, with optional per-step overrides. Meta user matching data is mapped by the flow, normalized and SHA-256 hashed server-side before it reaches `dataLayer`, and Meta browser identifiers are attached only to events with a Meta mapping. If a Meta event is present, the head also activates the authored Meta Pixel request proxy; GTM, Meta, and TrustedForm Partytown requests share the same central URL composer so worker-side requests are rewritten consistently.
