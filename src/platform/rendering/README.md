...
For faster transitions, production builds also emit a cacheable transition JS asset with static step HTML, non-PII client metadata, and registered behavior modules. The browser loads that asset after the first page settles; guarded checkpoint APIs still decide which route is allowed next.
